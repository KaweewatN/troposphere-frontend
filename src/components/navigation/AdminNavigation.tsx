import { House, Users2, CircleUserRound, University } from "lucide-react";
...
export default function AdminNavigation() {
  ...
  // Get clubId from URL params, location state, or admin membership
  const currentClubId =
    clubId ||
    (location.state as { clubId?: string })?.clubId ||
    memberships.find((m) => m.role === "ADMIN")?.club_id?.toString();
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-white text-neutral-400 shadow-lg max-w-screen-sm mx-auto">
      <ul className="flex justify-around items-center py-3 px-4">
        <NavItem
          to={`/admin/${currentClubId}/home`}
          icon={<House />}
          label="Home"
        />
        <NavItem
          to={`/admin/${currentClubId}/club-management`}
          icon={<University />}
          label="My Club"
        />
        <NavItem
          to={`/admin/${currentClubId}/member-management`}
          icon={<Users2 />}
          label="Members"
        />
        {/* <NavItem
          to={`/admin/${currentClubId}/approve-item`}
          icon={<CheckCircle2 />}
          label="Approve"
        /> */}
        <NavItem
